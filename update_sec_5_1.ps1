...
$new51Cells = @(
  ...
  @{
    ...
    "source"    = @(
      ...
      "`n",
      "**คำอธิบาย:** รับข้อมูลชื่อหุ้นและราคาในบรรทัดเดียวกัน เช่น `\"KTB 18.9 BBL 176.5 SCB 132.0 KBANK 172.0 TMB 1.8\"` แล้วรับชื่อหุ้นที่ต้องการค้นหาในบรรทัดถัดไป หากพบชื่อหุ้นให้แสดงราคาที่อยู่ถัดไป 1 ตำแหน่ง หากไม่พบให้แสดง `Not found``n"
    )
  },
  @{
    "cell_type" = "code"
...
# Keep cell 0 (Module Title), replace cells 1 to ($idx52 - 1) with new 5.1 cells, and keep everything from 5.2 onwards
$updatedCells = @($nb.cells[0]) + $new51Cells
...
Write-Host "UPDATED SECTION 5.1 IN MODULE 05 NOTEBOOK SUCCESSFULLY!"
